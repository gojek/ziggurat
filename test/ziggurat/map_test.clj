...
(deftest deep-merge-test
  (testing "when there is a value in map 'a' it returns value from 'a' "
    (let [a {:process-message-since-in-s 3600}
          b {:process-message-since-in-s 3500}
          expected-map {:process-message-since-in-s 3600}]
      (is (= (deep-merge a b) expected-map))))

  (testing "when a map is nil, take configuration from map b"
    (let [a {:process-message-since-in-s nil}
          b {:process-message-since-in-s 3500}
          expected-map {:process-message-since-in-s 3500}]
      (is (= (deep-merge a b) expected-map))))
