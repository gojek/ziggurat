...
(defn- mock-mapper-fn [{:keys [retry-counter-atom call-counter-atom retry-limit skip-promise success-promise]}]
  (fn [message metadata]
    (println "MOCK MAPPER FN MESSAGE >>>> " message)
    (println "MOCK MAPPER FN METADATA >>>> " metadata)
...
(deftest convert-and-ack-message-test
  (testing "While constructing a MessagePayload, adds topic-entity as a keyword and retry-count as 0 if message does not already has :retry-count"
    (let [message                   {:foo "bar"}
          ...
          converted-message-payload (consumer/convert-and-ack-message nil {:delivery-tag "delivery-tag"} (nippy/freeze message) false "default")]
