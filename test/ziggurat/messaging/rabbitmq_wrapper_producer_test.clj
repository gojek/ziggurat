(ns ziggurat.messaging.rabbitmq-wrapper-producer-test
      ...
      (with-redefs [config/ziggurat-config (constantly (assoc ziggurat-config
        ...
        (testing "it calls create queue for each queue creation with appropriate props"
          (let [counter               (atom 0)
            ...
            (with-redefs [rmqw/create-queue           (fn [queue props _]
                                                        (swap! counter inc)
                                                        (cond
                          ...
                          rmqw/declare-exchange       (fn [_ _] true)
                          rmqw/bind-queue-to-exchange (fn [_ _ _] true)]
              (producer/make-queues stream-routes)
              (is (= (* (count stream-routes) 3) @counter)))))
