...
(defn- producer-properties-from-config [producer-config]
...
(defstate kafka-producers
          :start (if (seq (producer-properties-map))
...
(defn send
  ...
  stream router config with key :default, use
  send like this.

  `(send :default \"test-topic\" \"value\")`
  `(send :default \"test-topic\" 1 \"key\" \"value\")`

  "

  ([stream-config-key topic data]
   (if (not-empty kafka-producers)
   ...
   ([ stream-config-key topic partition key data]
     (if (not-empty kafka-producers)
